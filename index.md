---
layout: home
---

## Hi, I am Bhavesh


### About Me
I hold a **Bachelor of Science in Information Technology** degree and am currently close to completing the **eJPT (eLearn Junior Penetration Tester)** certification.  
I aim to build a career in **Vulnerability Assessment & Penetration Testing (VAPT)** and **cloud security**.

I document my learning journey in offensive security through hands-on labs, CTFs, and practical experimentation.

This site primarily contains **TryHackMe write-ups** written from a **VAPT mindset**, focusing on:
- Enumeration-first methodology
- Understanding misconfigurations
- Learning from mistakes and dead ends
- Clear, reproducible steps rather than shortcuts

I intentionally keep these write-ups **raw and transparent** to reflect how real assessments unfold — not everything works on the first try, and that’s part of the process.

---


### 👤 Profiles
- **GitHub**: [massivelowtaperfade0](https://github.com/massivelowtaperfade0)  
- **TryHackMe**: [MrFade](https://tryhackme.com/p/Mr.Fade)
- **LinkedIn**: [kadam-bhavesh](https://www.linkedin.com/in/kadam-bhavesh/)

---

### 📓 Write-ups

#### TryHackMe
- [Boiler CTF — Medium](Write-Ups/TryHackMe/Boiler-ctf/)

---

### 🎯 Focus Areas
- Web & Linux enumeration  
- Privilege escalation fundamentals  
- Understanding vulnerable services  
- Building discipline through repetition  

---

### 💼 Open to Work

I am currently **open to entry-level roles, internships, or trainee positions** in:
- Penetration Testing
- SOC / Blue Team (foundational roles)
- Security Operations & Monitoring

I’m especially interested in roles that value **hands-on learning, documentation, and growth**.

---

> If you’re learning and feel stuck, that’s normal.  
> Progress comes from consistency, not perfection.
